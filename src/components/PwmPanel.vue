<template>
  <div id='PwmPanel'>
    <div class="ctrl">
      <div class="bg" @touchmove="onTouchMove($event)" @mousemove="onMouseMove($event)" @touchend="onMouseUp" ref="bg">
  ...
  methods: {
    ...
    onBaseMove({ clientX, clientY }) {
      ...
      if (x * x + y * y < this.bgSize * this.bgSize) {
        ...
        this.barOffset.x = Math.sin(angle) * this.bgSize;
        this.barOffset.y = Math.cos(angle) * this.bgSize;
      }
